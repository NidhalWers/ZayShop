...
def sendCode(username,email,code):
    ...
    Si vous n'avez pas essayé de vous connecter à partir d'un nouvel emplacement en France tout à l'heure, vous devez changer votre mot de passe immédiatement. Veuillez revérifier l'URL pour vous assurer qu'il s'agit de jay-app.com et non d'un autre domaine se faisant passer pour Jay.

    Vous ne devez en aucun cas copier ou partager le code ci-dessous.

    Si vous venez de vous connecter depuis un nouvel emplacement en France, puis en utilisant l'appareil à partir duquel vous venez d'essayer de vous connecter, veuillez entrer le code ci-dessous pour vous connecter à votre compte Jay :
    ''' +str(code) + '''

    Merci de faire confiance à Jay.

    --------------------------
    QuickBase: https://jay-app.com
    '''
